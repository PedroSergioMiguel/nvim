...
2.  **Clona esta configuración en tu directorio de configuración de NeoVim:**
    ```bash
    # Este repositorio contiene la carpeta 'nvim', que es la que debes usar.
    # Asumimos que has clonado este proyecto. Simplemente copia o mueve la carpeta 'nvim' a tu directorio de configuración.

    # En Linux/macOS
    cp -r path/to/this/repo/nvim ~/.config/

    # En Windows (PowerShell)
    Copy-Item -Path path\to\this\repo\nvim -Destination $env:LOCALAPPDATA\ -Recurse
    ```
